...
{
	table = Table( video_mode, table_file, ball_file, billiard_file );
...
void Game::NextTurn( sf::RenderWindow& window, int player_number )
{
    ...
    table.billiard[0].position = table.balls[CUE_BALL].position;
    hit_velocity = table.billiard[0].SetHit( window, table, score, player_number );
